  ...
  Type objective_function<Type>::operator() ()
{
	...
	//DATA_SCALAR(molt);					  // pre-recruit molt probability
	...
	PARAMETER(molt);								// molt probability of pre-recruit
	...
	// pre-recruits
	for (int i = 1; i < nyr; i++) {
	    pred_index(i,0) = (Type(1) - molt) * pred_index(i-1,0)*exp(-M) + exp(ln_rec(i-1));
	}
	for (int i = 1; i < nyr; i++) {
		for (int j = 1; j < nstage; j++) {
			// recruits
			if(j < nstage-1) {
			pred_index(i,j) = molt * pred_index(i-1,j-1)*exp(-M*tau_s(i-1));
			}
			// plus group
			if(j == nstage-1) {
